...
async function localDemo() {
	...
	console.log(
		`Workspace: ${identity.workspace.id} (role: ${identity.workspace.role})`,
	);
...
function startServer() {
	const server = http.createServer(async (req, res) => {
		try {
			...
			const firstMember = page.items[0]
				? await getUser(page.items[0].id, directoryOptions)
				: null;
			...
			res.end(
				JSON.stringify(
					{ user, identity, members: page.items, firstMember, groups },
					null,
					2,
				),
			);
